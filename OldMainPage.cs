...
namespace Calculadora
{
    public class OldMainPage : ContentPage
    {
        ...
        public OldMainPage()
        {
            ...
            layout.Children.Add(CriaOperadoIntervalorButton(".", 4, 0));

            Button zero = CreateNumberButton("0", 4, 1);
            ...
            Grid.SetColumnSpan(igual, 3);

            igual.Clicked += OnCalculate;
            layout.Children.Add(igual);

            OnClear(null, EventArgs.Empty);
            Content = layout;
        }

        private Button CriaOperadoIntervalorButton(string str, int row, int col)
        {
        ...
        void OnCalculate(object sender, EventArgs e)
        {
            if (currentState == 2)
            {
                var result = CalculadoraSimples.Calcular(primeiroNumero, segundoNumero, operador);
